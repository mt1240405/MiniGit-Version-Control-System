...
PointerIndexMap::PointerIndexMap(int initialCapacity)
{
...
size_t PointerIndexMap::MixHash(uintptr_t value) const
{
    value ^= value >> 33;
...
void PointerIndexMap::Rehash(int newCapacity)
{
    ...
    for (auto &entry : oldTable)
    {
        ...
        {
            size_t index = MixHash(entry.key) % capacity;
...
bool PointerIndexMap::Contains(void *pointer) const
{
    int dummy;
    return Get(pointer, dummy);
}

bool PointerIndexMap::Get(void *pointer, int &result) const
{
    uintptr_t key = reinterpret_cast<uintptr_t>(pointer);
    size_t index = MixHash(key) % capacity;
    size_t start = index;
...
void PointerIndexMap::Put(void *pointer, int value)
{
...
bool PointerIndexMap::Remove(void *pointer)
{
    uintptr_t key = reinterpret_cast<uintptr_t>(pointer);
    size_t index = MixHash(key) % capacity;
    size_t start = index;
